...
def count(test, pred, thresh, slice):
    ...
    for j in range(0, x_test):
        for k in range(0, y_test):
            if pred_thresh[slice][j][k] == 1 and test[slice][j][k] == 1:
                tp = tp + 1

            elif pred_thresh[slice][j][k] == 0 and test[slice][j][k] == 1:
                fn = fn + 1

            elif pred_thresh[slice][j][k] == 1 and 2 <= test[slice][j][k] <= 4:
                fp = fp + 1

            elif pred_thresh[slice][j][k] == 0 and 2 <= test[slice][j][k] <= 4:
                tn = tn + 1
...
def main():
    ...
    for thresh in range(thresh_s, thresh_e, threshold_step):
        ...
        with open(f'threshold_actin_slice{slice}.txt', 'a') as f:
