...
def count(test, pred, thresh, slice):
    ...
    for j in range(0, x_test):
        for k in range(0, y_test):
            if pred_thresh[slice][j][k] == 1 and test[slice][j][k] == 2:                    
                tp = tp + 1
                #print('tp')

            elif pred_thresh[slice][j][k] == 0 and test[slice][j][k] == 2:
                fn = fn + 1
                #print('fn')

            elif pred_thresh[slice][j][k] == 1 and  3 <= test[slice][j][k] <= 4:
                fp = fp + 1
                #print('fp')
            
            elif pred_thresh[slice][j][k] == 1 and test[slice][j][k] == 1:
                fp = fp + 1
                #print('fp')

            elif pred_thresh[slice][j][k] == 0 and 3 <= test[slice][j][k] <= 4:
                tn = tn + 1
                #print('tn')
                
            elif pred_thresh[slice][j][k] == 0 and test[slice][j][k] == 1:
                tn = tn + 1
                #print('tn')
...
def main():
    ...
    for thresh in range(thresh_s, thresh_e, threshold_step):
        ...
        sum_tp, sum_fn, sum_fp, sum_tn = count(test, pred, thresh, slice)

        precision, recall, iou, f1, f1_3,f1_5  = evaluate(sum_tp, sum_tn, sum_fp, sum_fn)

        with open(f'threshold_myosin_slice{slice}.txt', 'a') as f:
