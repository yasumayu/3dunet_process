    ...
    for i in range(0, z_test):
        for j in range(0, x_test):
            for k in range(0, y_test):
                if pred_thresh[i][j][k] == 1 and test[i][j][k] == 4:
                    tp = tp + 1
                    #print('tp')
                ...
                elif pred_thresh[i][j][k] == 1 and  1 <= test[i][j][k] <= 3:
                    fp = fp + 1
                    #print('fp')
                
                elif pred_thresh[i][j][k] == 0 and 1 <= test[i][j][k] <= 3:
                    tn = tn + 1
                    #print('tn')
...
def main():
    ...
    for thresh in range(thresh_s, thresh_e, threshold_step):
        ...
        with open('threshold_mitochon_multi.txt', 'a') as f:
